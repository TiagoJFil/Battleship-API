...
import pt.isel.daw.battleship.domain.Orientation.Horizontal
...
fun <T> T.toSiren(uri: String, uriVariables: Map<String, String>? = null): SirenEntity<T> {
    ...
    val links =
        get(Linking.LINKS, uri, uriVariables).map { it.toLink() } + listOf(selfLink(updateHref(uri, uriVariables)))
